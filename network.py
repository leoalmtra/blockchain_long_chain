        ...
        if msg["type"] == "block":
            ...
            if (
                ...
                if block.index <= len(blockchain) - 1:
                    print("[!] Fork detected — trying to resolve via longest chain rule...")

                    from chain import resolve_conflicts
                    resolve_conflicts(blockchain, blockchain_fpath,
                                    "configs/peers.txt", port)
        elif msg["type"] == "tx":
            tx = msg["data"]
            ...
            print(f"[i] Peer {addr} requested our chain. Sending...")
            blockchain_serializable = [b.as_dict() for b in blockchain]
            conn.send(json.dumps({"type": "full_chain", "data": blockchain_serializable}).encode())
    except Exception as e:
        print(
